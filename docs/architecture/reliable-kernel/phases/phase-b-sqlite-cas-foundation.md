...
- 创建单个 `limcode.sqlite`；
- 按 authority.json 的 75 个 Runtime domain exact set 建独立 table、Repository、Codec、mutation/client mapping 与 delete/reset policy；
...
- 75 个 target domain exact set 与 schema manifest 一致；
