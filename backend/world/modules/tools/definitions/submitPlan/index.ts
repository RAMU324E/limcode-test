...
Use this tool when the active workflow requires plan approval, or when a task involves non-trivial file edits, commands, or child agents. The plan field is the user-facing plan body. If useful, include taskList using the same shape as update_task_list so the approved plan can seed the conversation task list. In a root conversation, wait for the user's decision after calling submit_plan. In a delegated child Agent conversation, the reliable control plane automatically approves the Plan under the existing parent delegation and returns the decision directly. If the user requests changes in a root conversation, revise the plan and call submit_plan again.
