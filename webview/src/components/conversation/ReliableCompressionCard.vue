...
const title = computed(() => titleDetail.value?.status === 'ready' && titleDetail.value.text.trim()
  ...
  : envelope.value.trigger === 'auto' ? '自动上下文压缩' : '上下文压缩');
const methodLabel = computed(() => {
  switch (envelope.value.methodKind) {
...
const statusLabel = computed(() => status.value === 'enabled'
  ? '可用'
  : status.value === 'disabled' ? '已禁用' : status.value === 'soft_deleted' ? '已删除' : status.value);
...
function toggle(): void {
  ...
  feed.requestDetail('compression-title', blockId.value, { priority: 'expanded' });
  feed.requestDetail('compression-content', blockId.value, { priority: 'expanded' });
}
...
<template>
  <section
    ...
    :data-trigger="envelope.trigger || 'unknown'"
    :data-status="status"
  >
    <button type="button" class="compression-card-main" data-testid="compression-card-toggle" @click="toggle">
      <IconChevronRight class="compression-card-chevron" :class="{ expanded }" size="16" />
      ...
      <span class="compression-card-copy">
        <strong>{{ title }}</strong>
        <small>{{ methodLabel }}<template v-if="sourceCount !== undefined"> · {{ sourceCount }} 个上下文段</template><template v-if="savedTokens !== undefined"> · 节省约 {{ savedTokens }} tokens</template></small>
      </span>
      <span class="compression-card-status">{{ statusLabel }}</span>
    </button>
    <button
      type="button"
    ...
    <div v-if="expanded" class="compression-card-detail" data-testid="compression-detail">
